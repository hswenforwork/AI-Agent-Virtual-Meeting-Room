...
### Q6：上線前的舊訊息要顯示什麼？
- 問題：這次上線後才產生的訊息會有 token 數字，上線前已存在的舊訊息資料庫裡沒有這個
  資訊，要 (a) 完全不顯示（時間戳旁邊維持現狀，沒有多餘東西）、還是 (b) 顯示「無資料」
  之類的提示？
- 已記錄：(a) 完全不顯示，沒有資料的舊訊息時間戳旁邊維持現狀。

## 摘要／重要決策（更新）
1. 「這一輪」token = 產生這則訊息內容的那次呼叫，**不含**意圖分類呼叫本身；但如果分類
   結果是「task」，走工作型代理執行，執行過程中花的 token 要算進去（Managed Agents
   `session.usage` 事件可以拿到，技術上可行，見上方發現）。
2. 顯示格式：只顯示一個合計數字（例如「1,234 tokens」），不分輸入/輸出、不顯示模型名稱。
3. 適用範圍：一般聊天串流回覆 + task_card 任務卡片（含執行過程）；workspace_write 記事本/
   待辦短確認**不**顯示。
4. UI：常駐顯示在時間戳旁邊（例如「14:32 · 1,234 tokens」），不做 hover 才顯示。
5. 資料儲存：順便打底——欄位分開存 input/output tokens，即使畫面只顯示合計（3 的
   Q5）。技術細節（不需要另外確認，直接照決策 1-5 實作）：在 `messages` 表新增
   `input_tokens`／`output_tokens`（可為 null）欄位，`agent-run` 完成串流回覆時直接寫入
   同一則訊息；task_card 訊息則等工作型代理執行完成、`worker-task-start` 收到
   `session.usage` 事件時，把數字寫回對應的 task_card 訊息列。
6. 舊訊息（上線前已存在、沒有 token 資料）：完全不顯示，維持現狀。

...
### Q5：可以呼叫幾次
- 問題：一次任務執行過程中，這個工具可以被呼叫一次就好，還是可以呼叫多次（例如任務
  過程中陸續記錄好幾則）？
...
## 實作紀錄
- migration `0018_worker_task_notebook_tool.sql`：`worker_tasks` 新增 `needs_notebook_tool`。
- `workspaceWrite.ts`：`classifyMessage()` 的 `task` 分支多回傳 `needsNotebookTool`。
- `agent-run/index.ts`：分類出 `task` 時把這個欄位存進 `worker_tasks`。
- `managedAgents.ts`：抽出 `buildBaseTools()`／`buildSystemPrompt(includeNotebookTool)`
  共用邏輯；`createSession()` 新增 `includeNotebookTool` 參數，為 true 時用
  `agent_with_overrides` 幫這次 session 多附帶 `write_to_notebook` 工具跟對應的系統
  提示詞（單次 session 覆蓋，不影響 agent 本身固定的工具集/版本）。
- `worker-task-start/index.ts`：讀 `needs_notebook_tool`、算出房間的 `ownerId`
  （`resolveWorkspaceOwnerId()`）、把 `ownerId`／按下「開始執行」的 `userId` 一起放進
  `SessionContext`；事件迴圈新增 `write_to_notebook` 的處理（`handleWriteToNotebook()`），
  驗證輸入、呼叫既有的 `applyWorkspaceWrite()` 寫入，失敗時把錯誤回傳給模型、不讓整個
  任務算失敗（Q6），成功時順便推進度、讓任務卡片顯示到。

## 實作過程中順便發現並修正的既有 bug
`filesToArtifacts()`（工作型代理任務完成後把 `/mnt/session/outputs/` 底下的產出檔案
登記進 `files` 表那段邏輯）原本 insert 時完全沒有帶 `owner_id`/`created_by`。
`files.owner_id` 從 migration 0012 開始是 `not null`、預設值是 `auth.uid()`——這裡是用
service_role 呼叫，沒有登入 session、`auth.uid()` 一律是 `null`，這個 insert **從 0012
上線之後其實一直都會直接違反 not null 約束而失敗**：Storage 裡的檔案本體有成功上傳，
但資料庫這筆登記一直悄悄失敗（只有 `console.error`，不會讓整個任務失敗），檔案夾自然
永遠看不到這些產出檔案。這很可能就是稍早 `brainstorms/2026-09-23-worker-agent-
delegation-fixes.md` 記錄的「輸出檔案宣稱但檔案夾裡沒有」問題的真正根因之一
（對照組：一般使用者上傳走的 `file-register` 從一開始就有正確帶 `owner_id: user.id`，
只有工作型代理這條路徑被漏掉）。這次剛好已經把 `ownerId`／`userId` 帶進
`SessionContext`，順手把這個 insert 也補上 `owner_id: ctx.ownerId`／
`created_by: ctx.userId` 一併修正。

## 已知限制
`streamSessionEvents()`（`_shared/managedAgents.ts`）是另一份手刻的 SSE 解析邏輯，
沒有套用今天稍早在 `_shared/sse.ts` 修過的「連線結束前最後一個事件沒補結尾空行會被
吃掉」那個修正——理論上工作型代理的事件串流也可能踩到同一類問題，這次沒有一併處理，
之後如果任務執行遇到「明明有進度但事件收不到」的狀況，可以從這裡查起。

...
| AI-Partner 原做法 | 本專案資料表／操作流程 | 代理如何使用 | 如何驗證 |
|---|---|---|---|
| 只追加的 Markdown 清單：決策、原因、日期、替代方案、負責人；`/level-up` 第 2 階段自動寫入，也可手動加。 | 新表 `decisions`：`decision_text`／`reasoning`／`alternatives`／`decided_at`／`status`（`active`／`superseded`）／`supersedes_id`（自我參照）／`superseded_by_id`。新決策若帶 `supersedes_id`，DB trigger `handle_decision_supersede()` 會自動把舊決策標成 `superseded` 並回填 `superseded_by_id`——不是「悄悄覆蓋」，舊紀錄整列都還在，只是狀態換了。**「只追加」在資料庫層強制**（PR #44 四輪 review 修正）：真正的邊界是資料庫權限本身——`revoke update on public.decisions from authenticated, anon, service_role`（第四輪修正）——任何一般角色連 `UPDATE` 陳述式都會被資料庫直接拒絕，不會執行到任何 trigger 邏輯；`enforce_decision_append_only()` trigger 擋下 DELETE（連 service_role 都擋，不只是 RLS 層級），並作為第二層、非權威性的防禦保留 UPDATE 內容欄位／`status`／`superseded_by_id` 的判斷，但明確放行三種合法的系統級聯：
  - `status`／`superseded_by_id` 的變動：唯一合法路徑是 `handle_decision_supersede()`——這個函式改成 `security definer`，用函式擁有者（migration 執行者）的權限執行內部 UPDATE，不受上面的 `revoke` 影響。trigger 本身（第二層防禦）現在改成驗證「這個變化是否對應一筆真實存在、指回這一列的取代決策」，不再依賴任何可被一般角色自行設定的旗標——**第三輪版本原本用一個交易層級的 GUC（`app.decisions_supersede_update`）當旗標，但第四輪 review 用使用者提供的精確重現 SQL（`select set_config('app.decisions_supersede_update','true',true)` 後直接 `UPDATE`）實測證實：PostgreSQL 的 `app.*` 自訂參數任何角色都能自己設定，不是祕密或特權標記，這個旗標形同虛設，因此改用資料庫權限本身當真正邊界**。
  - `source_room_id` 允許被房間刪除的 `on delete set null` 改成 `null`（這個系統級聯已實測確認不受 `revoke update` 影響——FK 參照動作是系統層級操作，不受呼叫端自己在該表上的權限限制）。
  - `DELETE`：需要 `pg_trigger_depth() > 1`**且** `not exists(select 1 from profiles where id = old.owner_id)` 同時成立（後者才是真正對應「帳號正在被刪除」的條件——這一列的擁有者在交易當下查不到對應的 `profiles` 列；純用 `pg_trigger_depth() > 1` 只證明「巢狀在某個 trigger 裡」，不保證巢狀的原因就是帳號刪除，這是第三輪 review 指出的落差，這次沒有再改動）。`validate_decision_supersede()` trigger 在新增時用 `select ... for update` 鎖住目標決策，驗證它存在、屬於同一帳號、目前是 `active`，任一條件不成立就讓整筆新增操作失敗（不會留下孤兒的新決策）；這個鎖也同時擋下「兩筆決策同時搶著取代同一筆舊決策」的競態——第四輪修正時發現這個函式也需要改成 `security definer`，因為 `select ... for update`（`for share` 也一樣）在 PostgreSQL 裡本身就要求呼叫者對目標表有 `UPDATE` 權限，`revoke` 之後如果不改，會連合法的新增取代決策都做不到。 | `buildKnowledgeContext()` 只把 `status='active'` 的決策放進系統提示詞；若該決策取代過別的版本，附一句「此決策已取代：{舊決策標題}」。舊版決策仍查得到，但不會被當成目前依據餵給模型。 | 端到端測試第 4 點：對同一件事先做決策 D1，之後再做一個帶 `supersedes_id=D1` 的新決策 D2，確認模型下一次回覆只引用 D2、不再引用 D1；另外用兩個獨立資料庫連線同時對同一筆決策送出取代，確認只有一筆成功；第四輪新增：以一般登入角色重現使用者提供的旗標繞過 SQL，確認修正前 `UPDATE 1` 會成功、修正後變成 `permission denied for table decisions`（詳見測試報告 0.5 節）。 |
...
| AI-Partner 原做法 | 本專案資料表／操作流程 | 代理如何使用 | 如何驗證 |
|---|---|---|---|
| `connections.md`：記錄「這個系統知道有哪些外部工具（Gmail、日曆…），連線方式、授權、最近檢查時間」。這個角色在 AI-Partner 裡是「外部服務清單」。 | 本專案沒有第三方外部服務要連——「工具」本身就是這個聊天室自己的資料（訊息／記事／待辦／檔案）。所以借鏡的是 `connections.md` 的**治理精神**（有清單、有連線方式、有最近檢查時間、有效／失效要分開標示），落地成 `knowledge_sources` 表的 `source_type`（`message`／`note`／`task`／`file`／`external_url`）＋`status`（`valid`／`stale`／`invalid`）＋`last_checked_at`。**來源必須真的屬於同一帳號**（PR #44 第二輪 review 修正）：新增 `validate_knowledge_source_subject()` trigger，插入/更新一筆來源時驗證 `subject_id` 指到的知識/決策確實屬於這一列的 `owner_id`，避免任何人替別人的知識/決策塞入偽造的「已驗證」來源；`knowledgeContext.ts` 的 `fetchCitableSources()` 讀取端也明確加上 `owner_id` 過濾當第二層防護。 | 代理讀取知識時只看得到少數幾筆同一帳號、`status='valid'` 的來源（id、類型、驗證狀態、只有已驗證才附的摘要、畫面連結），不會把整個聊天記錄複製進提示詞（對應 `/link` 「不複製持續變動的事實，也不建立快速參考快取」）。 | 檢查 `knowledge_sources` 的筆數與 `buildKnowledgeContext()` 組出來的文字長度，確認沒有整段貼上原始訊息內容，且未驗證的來源不會附上摘要文字；另外測試 U2 直接對 U1 的知識插入一筆偽造來源，確認被 trigger 擋下（詳見測試報告）。 |
...
| 已確認事實才進正式頁面，並連回訪談紀錄（可追溯）。 | `accept_knowledge_proposal(proposal_id, edits jsonb)` RPC：使用者確認（可先編輯 `edits` 再送出）後，才真正 insert 進 `knowledge_items`／`decisions`／`knowledge_links`，同時把 `knowledge_proposals.resolved_knowledge_item_id`（或對應欄位）填回去、`status='accepted'`，正式知識列也留一個指回原提案的欄位，雙向可追溯。**來源摘要要真的由資料庫讀出來才附上**（PR #44 兩輪 review 修正）：`insert_message_source_if_owned()` 完全不接受任何呼叫端傳入的摘要／房間 id（第一輪修正只加了「確認同一帳號」，但仍然信任 RPC `edits` 參數傳進來的 `source_excerpt`——這個參數是呼叫 RPC 的使用者自己可以任意指定的，等於留了一個能捏造「已驗證」內容的管道；第二輪修正把這個參數整個拿掉），改成只留 `source_message_id` 當依據，函式內部直接 `join messages/rooms` 重新讀出當下的真實內容跟房間 id，同時確認訊息所在房間屬於這個提案的 `owner_id`；查不到或不是同一帳號就完全不附來源。 | 代理下次檢索到的是 `knowledge_items` 裡「已確認」的版本，不是提案草稿。 | 直接呼叫 RPC 測試：確認 accept 前後 `knowledge_items` 筆數、`knowledge_proposals.status` 的變化、兩邊互相連得回去；另外測試把 `source_message_id` 偽造成別人帳號的訊息，確認完全不會附上來源（詳見測試報告）。 |
...
| 可以手動跑，也建議每週固定跑。 | 手動：前端按鈕直接呼叫 `knowledge-audit`（帶使用者自己的登入 JWT，RLS 限制查詢範圍）。排程：`knowledge-audit/index.ts` 另外支援 `service_role` key＋明確 `ownerId` 參數的呼叫路徑（PR #44 review 修正：一開始的版本建議把某個使用者的 JWT 寫死進 `pg_cron` 排程 SQL，但 JWT 會過期，不是「可持續使用」的排程；`service_role` key 不會過期，才適合放進長期排程，但呼叫時仍然明確帶 `ownerId` 並在每張表的查詢都用它過濾，不是圖方便查全部人的資料）。README 附上用 Supabase Vault 存 `service_role` key、`pg_cron`＋`pg_net` 呼叫的完整 SQL，**但這個 PR 不會去啟用它**——依照指示，不直接改正式 Supabase、不啟用排程。 | — | 手動路徑的查詢邏輯用本機 PostgreSQL 實際驗證過；排程路徑（`service_role`＋`ownerId`）用同一份 `runAudit()` 函式，只是換了 client 來源，經程式碼檢視確認邏輯一致，但沒有真的透過 `service_role` 呼叫過一次（這個環境沒有可呼叫的 Supabase 專案）；排程本身留給使用者自行決定是否啟用，避免非預期的費用。 |
...
1. **語意相關度**：`buildKnowledgeContext()` 用「關鍵字重疊 → 更新時間」當相關度的替代方案，沒有做向量嵌入／語意搜尋——這跟現有 `buildNotesContext()`／`buildTasksContext()` 的作法一致（它們本來就是純依更新時間取最近幾筆，完全沒做相關度排序），這次選擇在既有基礎上加一層關鍵字比對，作為「相關」的最小可行版本，未來若要更準確需要另外引入 embedding，這次沒有做。**17 項修正計劃項目 13 修正**：候選範圍原本先限定「最近更新的 30 筆」才比對關鍵字，超過這個窗口的舊資料完全檢索不到；改成用 `pg_trgm` GIN 索引（`0020_knowledge_retrieval_indexes.sql`）讓 `ILIKE` 查詢可以用索引查整個帳號範圍，只有完全沒有關鍵字或比對不到任何一筆時才退回依更新時間排序——關鍵字重疊本身的比對方式沒有變，變的是「找的範圍」，見測試報告 0.6 節的實測（105 筆知識中，刻意放最舊的目標知識確認原本的 30 筆窗口找不到、新邏輯找得到）。
...
4. **排程稽核**：`knowledge-audit` 支援的 `service_role`＋`ownerId` 排程路徑只覆蓋單一帳號；多個使用者要各自排一份 `pg_cron` 工作，沒有另外做「幫所有帳號各跑一次」的批次版本。只提供「怎麼設定」的 SQL 與文件，不會在這個 PR 裡真的呼叫 Supabase 幫使用者打開；使用者要自己決定要不要、多常跑。
5. **知識來源的畫面連結，目前只有訊息型來源能真正跳轉並高亮**（`?highlight={messageId}`）：如果那則訊息不在聊天室目前已載入的分頁範圍內（比較舊、還沒往上捲載入），目前不會自動抓「訊息所在那一頁」再跳過去，使用者要自己往上捲找；記事／待辦／檔案型來源的連結只會開啟來源房間，沒有對應到單一項目的畫面內深連結。
6. **`knowledge-audit` 排程路徑（`service_role`＋`ownerId`）沒有實際呼叫測試過**：程式碼邏輯（跟手動路徑共用同一個 `runAudit()` 函式）經過檢視，但這個環境沒有可呼叫的 Supabase 專案，沒辦法真的用 `service_role` key 呼叫一次來確認。

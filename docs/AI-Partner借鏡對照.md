...
| AI-Partner 原做法 | 本專案資料表／操作流程 | 代理如何使用 | 如何驗證 |
|---|---|---|---|
| 只追加的 Markdown 清單：決策、原因、日期、替代方案、負責人；`/level-up` 第 2 階段自動寫入，也可手動加。 | 新表 `decisions`：`decision_text`／`reasoning`／`alternatives`／`decided_at`／`status`（`active`／`superseded`）／`supersedes_id`（自我參照）／`superseded_by_id`。新決策若帶 `supersedes_id`，DB trigger `handle_decision_supersede()` 會自動把舊決策標成 `superseded` 並回填 `superseded_by_id`——不是「悄悄覆蓋」，舊紀錄整列都還在，只是狀態換了。**「只追加」在資料庫層強制**（PR #44 review 修正）：`enforce_decision_append_only()` trigger 擋下任何 UPDATE 內容欄位或 DELETE（連 service_role 都擋，不只是 RLS 層級），但明確放行兩種合法的系統級聯：`source_room_id` 允許被房間刪除的 `on delete set null` 改成 `null`；`DELETE` 用 `pg_trigger_depth() > 1` 判斷這次刪除是不是從 `profiles` 的 `on delete cascade` 級聯觸發的（帳號被刪除時），是的話放行，直接對 `decisions` 下的 `DELETE` 一律擋下（第二輪 review 修正：第一版這兩種情況都會一併擋下，等於使用者刪除聊天室或刪除自己帳號都會失敗）。`validate_decision_supersede()` trigger 在新增時用 `select ... for update` 鎖住目標決策，驗證它存在、屬於同一帳號、目前是 `active`，任一條件不成立就讓整筆新增操作失敗（不會留下孤兒的新決策）；這個鎖也同時擋下「兩筆決策同時搶著取代同一筆舊決策」的競態。 | `buildKnowledgeContext()` 只把 `status='active'` 的決策放進系統提示詞；若該決策取代過別的版本，附一句「此決策已取代：{舊決策標題}」。舊版決策仍查得到，但不會被當成目前依據餵給模型。 | 端到端測試第 4 點：對同一件事先做決策 D1，之後再做一個帶 `supersedes_id=D1` 的新決策 D2，確認模型下一次回覆只引用 D2、不再引用 D1；另外用兩個獨立資料庫連線同時對同一筆決策送出取代，確認只有一筆成功（詳見測試報告）。 |

...
`DEFAULT_GPT_MODEL`（預設 `gpt-5.1`）、`DEFAULT_GEMINI_MODEL`（預設 `gemini-3.8-flash`）、
`DEFAULT_WORKER_AGENT_MODEL`（預設 `claude-opus-5`，工作型代理專用）是選用的，都只是使用者
還沒在「設定」頁選過模型時的兜底預設值，不填就用程式內建的預設值，一般部署不需要特別設定。

...
## 依序執行 migration —— 全新專案只需要 6 個檔案，不是全部照編號跑
...
supabase/migrations/0010_provider_model_selection.sql
```
...
`0001`／`0002`／`0007`／`0008`／`0009`／`0010` 六個檔案，跟這裡是一致的，照這六個檔案執行即可。
...
`0010_provider_model_selection.sql` 在同一張表加兩個欄位（使用者選的模型、快取的模型清單），
讓使用者可以從即時抓來的清單挑模型，不用再靠程式碼裡寫死、可能過期的模型名稱。
...
## 完成判斷
- 六個 migration 都成功執行（可以額外用 SQL 查 `select count(*) from public.rooms` 之類簡單語句驗證資料表確實存在）
